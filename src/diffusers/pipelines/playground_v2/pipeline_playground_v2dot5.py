        ...
        **kwargs,
        ...
        with swap_scheduler(self, edm_scheduler):  # XXXPGv2.5: swap in the monkey patched scheduler
            ...
            if not output_type == "latent":
                ...
                edm_mean = torch.tensor(self.vae.config.edm_mean).view(1, 4, 1, 1).to(device=latents.device, dtype=latents.dtype)
                edm_std = torch.tensor(self.vae.config.edm_std).view(1, 4, 1, 1).to(device=latents.device, dtype=latents.dtype)
